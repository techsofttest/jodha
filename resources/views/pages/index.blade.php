...
document.addEventListener("DOMContentLoaded", function () {
    ...
    @php $section1 = $home_sections->get(1) @endphp
    @if($section1 && $section1->cached_products?->count())
        <section class="product-section py-5">
            <div class="container-fluid px-4 px-lg-5 position-relative mb-4 d-flex justify-content-center align-items-center">
                <h2 class="section-title m-0">{{ $section1->title }}</h2>
                <a href="{{ $section1->view_all_url }}" class="view-all-link position-absolute end-0 me-4 me-lg-5">View all</a>
            </div>
            <div class="container-fluid px-4 px-lg-5">
                <div class="row mobile-swipe-row row-cols-md-3 row-cols-xl-5 g-4 pb-3">
                    @foreach($section1->cached_products as $product)
    ...
    {{-- ── Featured Product (shown after section 1) ────────────────────────── --}}
    @isset($home_product)
    <section class="container-fluid product-detail-section py-5 px-5">
        <div class="row">
            ...
            <div class="col-md-6 product-info-container font-jost px-lg-5 mt-4 mt-md-0">
                ...
                <p class="brand-text text-muted mb-4">{{ $home_product->subcategory?->subcat_name ?? 'Jodha' }}</p>
                ...
                <div class="attribute-section mb-5">
                    ...
                    <div class="attr-row mb-3 fw-light">
                        ...
                        <div class="attr-value text-muted d-inline-block pdp-attr-content">
                            {{ $home_product->material?->name ?? Str::limit(strip_tags($home_product->prod_material), 100) }}
                        </div>
                    </div>
                    @endif
                </div>

                @if($home_product->colors->count())
                <div class="variant-section mb-4">
                    ...
                    <div class="variant-options d-flex gap-2 fw-light align-items-center">
                        ...
                        <div id="homePdpColorPills" class="d-flex flex-wrap gap-2">
                            @foreach($home_product->colors as $color)
                            <button type="button" class="home-color-pill btn-color-pill"
                                data-color-id="{{ $color->id }}"
                                data-color-name="{{ $color->color_name }}"
                                title="{{ $color->color_name }}"
                                style="background:transparent;color:#222;border:2px solid #bbb;padding:6px 12px;border-radius:20px;">
                                {{ $color->color_name }}
                            </button>
                            @endforeach
                        </div>
                ...
                <div class="variant-section mb-5">
                    ...
                    <div class="variant-options d-flex gap-2 fw-light">
                        <select id="pdpSizeSelect" class="form-select rounded-0 border text-muted px-3 py-2"
                            style="width:auto;min-width:200px;font-size:14px;background:var(--bg-color);">
                ...
                <div class="cta-buttons d-grid gap-3">
                    <button id="homeAddToCartBtn" data-product-id="{{ $home_product->id }}"
                        class="btn btn-outline-dark add-to-cart-btn btn-lg rounded-0 fw-light text-uppercase">Add to cart</button>
                    <a href="{{ route('product.show', $home_product->prod_slug) }}"
                        class="btn solid-gold-btn btn-lg buy-now-btn rounded-0 fw-light text-uppercase text-center text-decoration-none">View full details</a>
                </div>
            </div>
        </div>
    </section>
    ...
    {{-- ═══ Dead block below — kept only for reference, safe to delete ═══ --}}
    @if(false)
    ...
    @php $section2 = $home_sections->get(2) @endphp
    @if($section2 && $section2->cached_products?->count())
        <section class="product-section py-5">
            <div class="container-fluid px-4 px-lg-5 position-relative mb-4 d-flex justify-content-center align-items-center">
                <h2 class="section-title m-0">{{ $section2->title }}</h2>
                <a href="{{ $section2->view_all_url }}" class="view-all-link position-absolute end-0 me-4 me-lg-5">View all</a>
            </div>
            <div class="container-fluid px-4 px-lg-5">
                <div class="row mobile-swipe-row row-cols-md-3 row-cols-xl-5 g-4 pb-3">
                    @foreach($section2->cached_products as $product)
                        @include('components.product-card', ['product' => $product])
                    @endforeach
                </div>
            </div>
    ...
    @php $section3 = $home_sections->get(3) @endphp
    @if($section3 && $section3->cached_products?->count())
        <section class="product-section py-5">
            <div class="container-fluid px-4 px-lg-5 position-relative mb-4 d-flex justify-content-center align-items-center">
                <h2 class="section-title m-0">{{ $section3->title }}</h2>
                <a href="{{ $section3->view_all_url }}" class="view-all-link position-absolute end-0 me-4 me-lg-5">View all</a>
            </div>
            <div class="container-fluid px-4 px-lg-5">
                <div class="row mobile-swipe-row row-cols-md-3 row-cols-xl-5 g-4 pb-3">
                    @foreach($section3->cached_products as $product)
                        @include('components.product-card', ['product' => $product])
                    @endforeach
                </div>
            </div>
    ...
    {{-- ── Section 4 (display_order = 4) ───────────────────────────────────── --}}
    @php $section4 = $home_sections->get(4) @endphp
    @if($section4 && $section4->cached_products?->count())
        <section class="product-section py-5">
            <div class="container-fluid px-4 px-lg-5 position-relative mb-4 d-flex justify-content-center align-items-center">
                <h2 class="section-title m-0">{{ $section4->title }}</h2>
                <a href="{{ $section4->view_all_url }}" class="view-all-link position-absolute end-0 me-4 me-lg-5">View all</a>
            </div>
            <div class="container-fluid px-4 px-lg-5">
                <div class="row mobile-swipe-row row-cols-md-3 row-cols-xl-5 g-4 pb-3">
                    @foreach($section4->cached_products as $product)
                        @include('components.product-card', ['product' => $product])
                    @endforeach
                </div>
            </div>
    ...
    {{-- ── Section 5 (display_order = 5) ───────────────────────────────────── --}}
    @php $section5 = $home_sections->get(5) @endphp
    @if($section5 && $section5->cached_products?->count())
        <section class="product-section py-5">
            <div class="container-fluid px-4 px-lg-5 position-relative mb-4 d-flex justify-content-center align-items-center">
                <h2 class="section-title m-0">{{ $section5->title }}</h2>
                <a href="{{ $section5->view_all_url }}" class="view-all-link position-absolute end-0 me-4 me-lg-5">View all</a>
            </div>
            <div class="container-fluid px-4 px-lg-5">
                <div class="row mobile-swipe-row row-cols-md-3 row-cols-xl-5 g-4 pb-3">
                    @foreach($section5->cached_products as $product)
                        @include('components.product-card', ['product' => $product])
                    @endforeach
                </div>
            </div>
