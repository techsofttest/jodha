...
@section('content')
    ...
    <section class="page-title-section text-center py-5" style="background-color: var(--c-linen);">
        <div class="container mt-4">
            <h1 class="font-heading mb-3 text-uppercase letter-spacing-2" style="color: var(--c-primary);">{{ $subcategory->name }}</h1>
            <nav aria-label="breadcrumb">
                <ol class="breadcrumb justify-content-center mb-0" style="font-family: var(--f-body); font-size: 13px;">
                    <li class="breadcrumb-item"><a href="{{ route('home') }}" class="text-decoration-none text-muted">Home</a></li>
                    @if($subcategory->category)
                        <li class="breadcrumb-item"><a href="{{ route('category.show', $subcategory->category->slug) }}" class="text-decoration-none text-muted">{{ $subcategory->category->name }}</a></li>
                    @endif
